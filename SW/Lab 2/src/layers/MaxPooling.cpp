...
namespace ML
{
    ...
    // Debug message
    std::cout << "[DEBUG] MaxPooling computeQuantized() called" << std::endl;
    
    // MaxPooling per lab specs: "take the max of the int8 input values without 
    // any dequantization/requantization involved"
    // Translation: MaxPooling works the same way regardless of data type
    // Just find the maximum value - it doesn't matter if it's fp32 or quantized
    ...
    // Max pooling computation - works identically for fp32 or quantized data
    // No quantization/dequantization needed - just find max values
    for (size_t c = 0; c < outputChannels; c++) {
        for (size_t h_out = 0; h_out < outputHeight; h_out++) {
            for (size_t w_out = 0; w_out < outputWidth; w_out++) {
                fp32 maxVal = -INFINITY;
    ...
    std::cout << "[DEBUG] First 3 maxpool outputs: " 
              << output.get<fp32>(0) << ", " 
              << output.get<fp32>(1) << ", " 
